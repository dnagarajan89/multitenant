		...
		return TransactionSynchronizationManager.forCurrentTransaction()
				...
				.flatMap(synchronizationManager -> {
					return doGetSession(synchronizationManager, factory, sessionSynchronization) //
							.flatMap(it -> getDatabaseName(dbName)
									.map(databaseName -> getMongoDatabaseOrDefault(databaseName, factory.withSession(it)))
							);
				})
				.onErrorResume(NoTransactionException.class,
						e -> getDatabaseName(dbName).map(databaseName -> getMongoDatabaseOrDefault(databaseName, factory)))
				.switchIfEmpty(getDatabaseName(dbName).map(databaseName -> getMongoDatabaseOrDefault(databaseName, factory)));
	}

	private static Mono<String> getDatabaseName(String defaultDbName) {
		return Mono
				.subscriberContext()
				.filter(context -> context.hasKey(TENANT_KEY))
				.map(context -> context.get(TENANT_KEY))
				.map(String::valueOf)
				.defaultIfEmpty(defaultDbName != null ? defaultDbName : "");
	}
